...
export default function AdminUserDetail() {
  ...
  const handleStatusToggle = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'reactdjango saved the account status change.',
        variant: 'success',
      })
    } catch (requestError) {
      toast({
        title: 'Status update failed',
        description: requestError.response?.data?.detail || 'reactdjango could not update this user right now.',
        variant: 'error',
      })
    } finally {
  ...
  const handlePlanSave = async () => {
    ...
    try {
      ...
      toast({
        title: 'Plan updated',
        description: 'reactdjango applied the manual plan override.',
        variant: 'success',
      })
    } catch (requestError) {
      toast({
        title: 'Plan change failed',
        description: requestError.response?.data?.detail || 'reactdjango could not change the plan right now.',
        variant: 'error',
      })
    } finally {
      setSavingPlan(false)
  ...
  const handleSendReset = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'reactdjango emailed a secure password reset link to the user.',
        variant: 'success',
      })
    } catch (requestError) {
      toast({
        title: 'Reset email failed',
        description: requestError.response?.data?.detail || 'reactdjango could not send the reset email.',
        variant: 'error',
      })
    } finally {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      toast({
        title: 'User deleted',
        description: response.message || 'reactdjango permanently deleted this user account.',
        variant: 'success',
      })
      ...
      toast({
        title: 'Delete failed',
        description: requestError.response?.data?.detail || 'reactdjango could not delete this user right now.',
        variant: 'error',
      })
    } finally {
